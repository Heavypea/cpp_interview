...
- **new[]和delete[]**
  ...
  我们再用new[]创建数组的时候，比如一个对象大小为N，则K个对象需要K*N个空间来构造。那当delete的时候如何知道这个数组空间的大小呢？我们会在new出来的这个空间的头部申请一个int类型的字节，4字节用来存储数组的长度，这样调用delete[]的时候就知道数组的大小，才会调用K次析构函数，释放K * N + 4字节大小的内存。
...
首先不是BUG，这种情况属于**未定义的行为**，而未定义的行为不会导致编程失败，但可能会造成程序崩溃。
...
3. **const_cast**
   ...
   但是不能用于去除变量的常量性，而是去除指向对象的引用或指针的常量性，因此去除对象必须是指针或者引用。https://blog.csdn.net/Dasis/article/details/121343011#3const_cast_63
...
2. 类中有对象或者继承关系时（没有默认构造函数的情况下）。首先要明确一点就是如果一个类有自定义的构造函数时后，编译器就不会创建默认构造函数了。 
   ...
   还有继承的时候也是，创建派生类对象时会先创建基类对象，调用基类的构造函数，如果基类有自定义的构造函数的话，你没有列表初始化，编译器会调用默认构造函数，就会出错
...
### 内存池概述
https://blog.csdn.net/A642960662/article/details/123029536
